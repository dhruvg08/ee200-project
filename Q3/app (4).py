...
import matplotlib
matplotlib.use('Agg')  # non-interactive backend, uses less memory
    ...
    if uploaded_file is not None:
        if not db:
            ...
            try:
                ...
                del file_bytes  # free immediately after writing
                gc.collect()
                ...
                try:
                    ...
                    ax2.set_title(f"Constellation Map ({len(q_peaks)} peaks)", color='white')
                    ...
                    st.pyplot(fig1)
                    plt.close(fig1)
                    del f_arr, t_arr, Sxx, Sxx_db, fig1  # free memory
                    ...
                    if prediction and offsets is not None and len(offsets) > 0:
                        ...
                        ax.set_title(f"Offset Histogram for Match: {prediction}", color='white')
            ...
            with st.spinner("Processing batch archive..."):
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    for root, _, files in os.walk(extract_dir):
                        for fname in files:
                            if fname.lower().endswith(('.mp3', '.wav')) and not fname.startswith('.'):
                    ...
                    if not audio_files:
                        ...
                        for idx, filepath in enumerate(audio_files):
                            ...
                            try:
                                ...
                                pred, v, _, _ = db.match_clip(audio)
                                results.append({"filename": fname, "prediction": pred if pred else ""})
                            except Exception as e:
                                results.append({"filename": fname, "prediction": ""})
                            finally:
                                if audio is not None:
                                    del audio
                                gc.collect()  # free memory after each song

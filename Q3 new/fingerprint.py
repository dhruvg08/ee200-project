...
class SongDatabase:
    ...
    def match_clip(self, audio: np.ndarray, sr: int = TARGET_SR):
        ...
        # Best match = song whose single best offset has the most votes
        best_id, best_votes = -1, 0
        for sid, off_counts in votes.items():
            peak_cnt = max(off_counts.values())
            if peak_cnt > best_votes:
                best_votes = peak_cnt
                best_id    = sid
